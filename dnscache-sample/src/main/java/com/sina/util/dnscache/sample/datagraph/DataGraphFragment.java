package com.sina.util.dnscache.sample.datagraph;
...
import com.sina.util.dnscache.sample.R;
import com.sina.util.dnscache.sample.simulationtask.TaskManager;
import com.sina.util.dnscache.sample.simulationtask.TaskModel;
...
public class DataGraphFragment extends Fragment{
    ...
    public String[] serverName( ArrayList<TaskModel> list ){
		...
		Hashtable< String, ArrayList<ServerSpeedActivity.DataRate> > serverIpSpeedList = new Hashtable<String, ArrayList<ServerSpeedActivity.DataRate>>() ;
		for( TaskModel taskModel : list ){
			ArrayList<ServerSpeedActivity.DataRate> ips = serverIpSpeedList.get(taskModel.hostIp) ;
			if( ips == null ){
				ips = new ArrayList<ServerSpeedActivity.DataRate>() ;
				serverIpSpeedList.put(  taskModel.hostIp , ips ) ; 
			}
			if (null != taskModel.hostInputStreamResult) {
			    ips.add(new ServerSpeedActivity.DataRate(taskModel.hostInputStreamResult.length, taskModel.hostExpendTime + taskModel.httpDnsExpendTime)) ;
            }
			...
			if (null != taskModel.domainInputStreamResult) {
			    ips.add(new ServerSpeedActivity.DataRate(taskModel.domainInputStreamResult.length, taskModel.domainExpendTime)) ;
			}
		}
		...
		for(Iterator<String> itr = serverIpSpeedList.keySet().iterator(); itr.hasNext();){
			...
			ArrayList<ServerSpeedActivity.DataRate> value = (ArrayList<ServerSpeedActivity.DataRate>) serverIpSpeedList.get(key);
			ip.add(key) ; 
			long sunSize = 0 ; 
			long sunElapsed = 1 ; 
			for( ServerSpeedActivity.DataRate l : value ){
